...
## [RocketAi] makes is built on it, and it is deliberately a simplification:
...
func test_the_ball_is_the_published_size_and_weight() -> void:
	assert_almost_eq(ball.mass, RocketConst.BALL_MASS, 0.01, "BALL_MASS_BT is a sixth of a car")
	var shape: CollisionShape3D = ball.get_node(^"CollisionShape3D")
	var sphere: SphereShape3D = shape.shape as SphereShape3D
	assert_almost_eq(sphere.radius, RocketConst.BALL_RADIUS, 0.001, "91.25 uu")
...
func test_drag_is_set_the_way_rocket_league_models_it() -> void:
	...
	assert_almost_eq(ball.linear_damp, RocketConst.BALL_DRAG, 0.0001)


func test_it_bounces_at_sixty_percent() -> void:
	assert_almost_eq(ball.physics_material_override.bounce, RocketConst.BALL_RESTITUTION, 0.001)
	assert_almost_eq(ball.physics_material_override.friction, RocketConst.BALL_FRICTION, 0.001)


func test_speed_is_capped_at_the_published_maximum() -> void:
	ball.linear_velocity = Vector3(500.0, 500.0, 500.0)
	await wait_physics_frames(3)
	assert_lte(ball.linear_velocity.length(), RocketConst.BALL_MAX_SPEED + 0.5,
...
func test_spin_is_capped_at_the_published_maximum() -> void:
	...
	await wait_physics_frames(3)
	assert_lte(ball.angular_velocity.length(), RocketConst.BALL_MAX_ANGULAR + 0.2,
...
func test_a_reset_puts_it_somewhere_still() -> void:
	...
	ball.reset_to(Vector3(0.0, RocketConst.BALL_REST_HEIGHT, 0.0))
	await wait_physics_frames(2)
...
func test_the_prediction_never_leaves_the_floor_or_the_ceiling() -> void:
	...
	for ahead: float in [0.5, 1.0, 2.0, 4.0]:
		...
		assert_gte(guess.y, RocketConst.BALL_RADIUS - 0.01,
			"The ball cannot be predicted through the floor, at %s seconds" % ahead)
		assert_lte(guess.y, RocketConst.ARENA_HEIGHT, "Nor through the roof")
...
func test_the_prediction_keeps_the_ball_inside_the_side_walls() -> void:
	...
	for ahead: float in [0.5, 1.0, 2.0, 3.0]:
		var guess: Vector3 = ball.predict(ahead)
		assert_lte(absf(guess.x), RocketConst.ARENA_EXTENT_X + 0.01,
...
func test_the_prediction_lets_a_ball_through_the_goal_mouth() -> void:
	...
	assert_gt(guess.z, RocketConst.ARENA_EXTENT_Y,
...
func test_the_prediction_is_capped_rather_than_running_forever() -> void:
	...
	var capped: Vector3 = ball.predict(RocketBall.PREDICT_MAX_TIME)
