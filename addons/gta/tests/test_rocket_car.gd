...
func test_it_starts_with_the_published_kickoff_tank() -> void:
	assert_almost_eq(car.boost, RocketConst.BOOST_START, 0.01,
...
func test_a_pad_tops_the_tank_up_without_going_over() -> void:
	...
	car.collect_boost(RocketConst.PAD_BOOST_SMALL)
	assert_almost_eq(car.boost, 62.0, 0.01, "A small pad is worth 12")
	car.collect_boost(RocketConst.PAD_BOOST_BIG)
	assert_almost_eq(car.boost, RocketConst.BOOST_MAX, 0.01, "A big pad fills it and stops there")
...
func test_holding_boost_spends_the_tank() -> void:
	...
	assert_lt(car.boost, RocketConst.BOOST_MAX, "Holding boost spends it")
...
func test_a_car_on_the_ground_jumps_when_the_button_goes_down() -> void:
	car.place_at(Vector3(0.0, RocketConst.CAR_REST_HEIGHT, 0.0), 0.0)
...
func test_the_two_teams_attack_opposite_ends() -> void:
	...
	car.team = RocketCar.Team.ORANGE
...
func test_the_sticky_force_presses_the_car_into_flat_ground() -> void:
	...
	assert_almost_eq(car.global_position.y, RocketConst.CAR_REST_HEIGHT, 0.2,
...
func test_the_sticky_force_is_stronger_the_steeper_the_surface() -> void:
	...
	var flat: float = RocketConst.STICKY_FORCE_BASE + (1.0 - 1.0)
	var wall: float = RocketConst.STICKY_FORCE_BASE + (1.0 - 0.0)
	...
	assert_gt(wall * RocketConst.GRAVITY, RocketConst.GRAVITY,
...
func test_a_dodge_takes_the_pitch_axis_away_for_a_moment() -> void:
	...
	assert_almost_eq(car._pitchlock_time, RocketConst.FLIP_PITCHLOCK_TIME, 0.2,
		"And it runs for the published second")


func test_the_pitch_lock_clears_on_a_kickoff() -> void:
	car._pitchlock_time = RocketConst.FLIP_PITCHLOCK_TIME
	car.place_at(Vector3(0.0, RocketConst.CAR_REST_HEIGHT, 0.0), 0.0)
...
func test_a_dodge_from_rest_is_worth_the_same_whichever_way_it_is_thrown() -> void:
	...
	for gained: float in speeds:
		assert_almost_eq(gained, RocketConst.FLIP_INITIAL_VEL, 2.0,
...
func test_ball_cam_locks_the_chase_camera_onto_the_ball() -> void:
	var ball: RocketBall = load("res://addons/gta/scenes/rocket_ball.tscn").instantiate()
