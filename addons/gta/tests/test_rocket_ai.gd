...
const ROCKET_BALL: PackedScene = preload("res://addons/gta/scenes/rocket_ball.tscn")
...
func _add_car(team: RocketCar.Team, at: Vector3) -> RocketCar:
	var car: RocketCar = ROCKET_CAR.instantiate()
	car.team = team
	var brain: RocketAi = car.get_node(^"RocketAi")
	brain.enabled = true
...
func _brain(car: RocketCar) -> RocketAi:
	return car.get_node(^"RocketAi") as RocketAi


func test_switching_the_brain_on_tells_the_car_an_ai_has_the_wheel() -> void:
	var car: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 20.0))
	await wait_physics_frames(2)
...
func test_a_lone_car_attacks() -> void:
	...
	assert_eq(_brain(car).role, RocketAi.Role.ATTACK,
...
func test_the_nearer_car_takes_the_ball() -> void:
	...
	var near: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 8.0))
	var far: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 44.0))
	await wait_physics_frames(6)
	assert_eq(_brain(near).role, RocketAi.Role.ATTACK)
	assert_ne(_brain(far).role, RocketAi.Role.ATTACK, "Two cars do not chase the same ball")


func test_the_deepest_of_three_defends() -> void:
	ball.global_position = Vector3(0.0, 1.0, -30.0)
	var first: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, -20.0))
	var second: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(6.0, 0.2, 0.0))
	var third: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(-6.0, 0.2, -44.0))
	await wait_physics_frames(6)
	var roles: Array[int] = [_brain(first).role, _brain(second).role, _brain(third).role]
	assert_true(roles.has(RocketAi.Role.ATTACK), "Somebody has to go for it")
	assert_true(roles.has(RocketAi.Role.DEFEND), "And somebody has to stay home")


func test_the_roles_rotate_when_the_ball_moves() -> void:
	ball.global_position = Vector3(0.0, 1.0, 30.0)
	var one: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 34.0))
	var two: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, -34.0))
	await wait_physics_frames(6)
	assert_eq(_brain(one).role, RocketAi.Role.ATTACK)
	# send the ball to the other end; the other car is now the closer of the two
	ball.global_position = Vector3(0.0, 1.0, -30.0)
	await wait_physics_frames(6)
	assert_eq(_brain(two).role, RocketAi.Role.ATTACK,
		"Roles follow the ball rather than being handed out once")


func test_an_opponent_is_never_counted_as_a_team_mate() -> void:
	ball.global_position = Vector3(0.0, 1.0, 0.0)
	var blue: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 30.0))
	_add_car(RocketCar.Team.ORANGE, Vector3(0.0, 0.2, 2.0))
	await wait_physics_frames(6)
	assert_eq(_brain(blue).role, RocketAi.Role.ATTACK,
...
func test_the_two_teams_aim_from_opposite_sides_of_the_ball() -> void:
	ball.global_position = Vector3(0.0, 1.0, 0.0)
	var blue: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 20.0))
	var orange: RocketCar = _add_car(RocketCar.Team.ORANGE, Vector3(0.0, 0.2, -20.0))
	await wait_physics_frames(6)
...
func test_a_kickoff_is_recognised_by_the_ball_sitting_on_the_spot() -> void:
	...
	ball.freeze = true
	ball.global_position = Vector3(0.0, RocketConst.BALL_REST_HEIGHT, 0.0)
	ball.linear_velocity = Vector3.ZERO
	var car: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 25.6))
	await wait_physics_frames(6)
...
func test_it_will_not_detour_across_the_pitch_for_a_pad() -> void:
	var car: RocketCar = _add_car(RocketCar.Team.BLUE, Vector3(0.0, 0.2, 0.0))
	await wait_physics_frames(4)
	var pad: RocketBoostPad = load("res://addons/gta/scenes/rocket_boost_pad.tscn").instantiate()
	pad.is_big = true
