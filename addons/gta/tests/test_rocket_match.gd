...
func before_each() -> void:
	...
	var replay: RocketReplay = RocketReplay.new()
	replay.name = "Replay"
	...
	var arena: RocketArena = RocketArena.new()
	arena.name = "Arena"
	match_node.add_child(arena)
	var ui: RocketLeagueUi = load("res://addons/gta/scenes/rocket_league_ui.tscn").instantiate()
	ui.name = "UI"
...
func test_the_ball_on_the_centre_spot_is_not_a_goal() -> void:
	ball.global_position = RocketMatch.CENTRE_SPOT
	assert_eq(match_node._goal_crossed(), 0)


func test_a_ball_over_the_line_between_the_posts_is_a_goal() -> void:
	ball.global_position = Vector3(0.0, 1.0, RocketConst.GOAL_LINE_Y + 0.5)
...
func test_a_ball_just_short_of_the_line_is_not_a_goal() -> void:
	ball.global_position = Vector3(0.0, 1.0, RocketConst.GOAL_LINE_Y - 0.05)
	assert_eq(match_node._goal_crossed(), 0,
		"Rocket League needs the ball wholly over 5124.25 uu")


func test_a_ball_wide_of_the_post_is_not_a_goal() -> void:
	ball.global_position = Vector3(RocketConst.GOAL_HALF_WIDTH + 1.0, 1.0,
		RocketConst.GOAL_LINE_Y + 0.5)
...
func test_a_ball_over_the_crossbar_is_not_a_goal() -> void:
	ball.global_position = Vector3(0.0, RocketConst.GOAL_HEIGHT + 1.0,
		RocketConst.GOAL_LINE_Y + 0.5)
	assert_eq(match_node._goal_crossed(), 0)
...
func test_a_goal_in_blue_s_net_is_scored_by_orange() -> void:
	...
	match_node._award_goal(RocketCar.Team.ORANGE)
...
func test_the_clock_counts_down_and_stops_at_zero() -> void:
	...
	match_node.state = RocketMatch.State.PLAYING
	...
	assert_ne(match_node.state, RocketMatch.State.OVER,
...
func test_full_time_with_a_lead_ends_the_match() -> void:
	...
	assert_eq(match_node.state, RocketMatch.State.OVER)
...
func test_the_first_goal_in_overtime_ends_it() -> void:
	...
	match_node.orange_score = 1
	watch_signals(match_node)
	match_node._award_goal(RocketCar.Team.BLUE)
	assert_eq(match_node.state, RocketMatch.State.OVER, "Golden goal")
	assert_signal_emitted(match_node, "match_ended")
...
func test_the_replay_camera_is_aimed_from_inside_the_arena() -> void:
	...
	for goal_z: float in [RocketConst.GOAL_LINE_Y, -RocketConst.GOAL_LINE_Y]:
		...
		assert_lt(absf(eye.x), RocketConst.ARENA_EXTENT_X,
			"Inside the side walls, filming the goal at %.1f" % goal_z)
		assert_lt(absf(eye.z), RocketConst.ARENA_EXTENT_Y,
			"Inside the back walls, filming the goal at %.1f" % goal_z)
		assert_between(eye.y, 0.0, RocketConst.ARENA_HEIGHT,
...
func test_the_replay_camera_looks_back_at_the_goal_it_is_filming() -> void:
	match_node._aim_replay_at(-RocketConst.GOAL_LINE_Y)
	await wait_physics_frames(3)
	var eye: Vector3 = match_node.replay_camera.global_position
...
func test_a_kickoff_counts_down_before_anyone_may_drive() -> void:
	match_node.start_kickoff()
	assert_eq(match_node.state, RocketMatch.State.COUNTDOWN)
...
func test_nobody_is_driving_while_the_clock_is_stopped() -> void:
	...
	for frozen: RocketMatch.State in [RocketMatch.State.COUNTDOWN,
			RocketMatch.State.REPLAY, RocketMatch.State.OVER]:
		match_node.state = frozen
		assert_ne(match_node.state, RocketMatch.State.PLAYING)
