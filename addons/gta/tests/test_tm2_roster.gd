...
func test_every_car_has_a_complete_stat_block() -> void:
	assert_eq(Tm2Roster.CARS.size(), 14, "The Twisted Metal 2 roster is fourteen cars")
	for car: StringName in Tm2Roster.CARS:
...
func test_the_published_outliers_are_what_the_tables_say() -> void:
	...
	assert_eq(float(Tm2Roster.stats(&"minion")["health"]), 230.0)
	assert_eq(float(Tm2Roster.stats(&"mr_grimm")["health"]), 85.0)
	assert_eq(float(Tm2Roster.stats(&"twister")["top_mph"]), 140.0)
	assert_eq(float(Tm2Roster.stats(&"roadkill")["health"]), 120.0)


func test_top_speed_converts_the_published_mph_to_metres_per_second() -> void:
	assert_almost_eq(Tm2Roster.top_speed(&"roadkill"), 117.0 * 0.44704, 0.001)
	assert_almost_eq(Tm2Roster.top_speed(&"roadkill", true), 156.0 * 0.44704, 0.001)
	assert_gt(Tm2Roster.top_speed(&"twister"), Tm2Roster.top_speed(&"sweet_tooth"),
		"Twister is the quicker car")


func test_an_unknown_car_reports_nothing_rather_than_a_made_up_number() -> void:
	assert_eq(Tm2Roster.stats(&"not_a_car"), {})
	assert_eq(Tm2Roster.top_speed(&"not_a_car"), 0.0)


func test_weapon_damage_matches_the_published_figures() -> void:
	assert_eq(float(Tm2Roster.WEAPONS[&"fire_missile"]["damage"]), 7.0)
	assert_eq(float(Tm2Roster.WEAPONS[&"homing_missile"]["damage"]), 10.0)
	assert_eq(float(Tm2Roster.WEAPONS[&"power_missile"]["damage"]), 15.0)
	assert_eq(float(Tm2Roster.WEAPONS[&"remote_bomb"]["damage"]), 25.0)
	# the ricochet is the one weapon that grows as it bounces
	assert_gt(float(Tm2Roster.WEAPONS[&"ricochet"]["max_damage"]),
		float(Tm2Roster.WEAPONS[&"ricochet"]["damage"]))


func test_the_numbers_that_could_not_be_sourced_are_listed() -> void:
	assert_gt(Tm2Roster.UNSOURCED.size(), 0,
