class_name RlAi
extends Node
## Drives a [RlCar] through a game of soccar.
##
## It is built the way [TwAi] is: it never touches the handling model, it
## fills in a virtual control pad and hands it to [method
## RlCar.set_rocket_input] once a physics frame. It has the same eight
...
## pretending to: [constant RlConst.UNSOURCED] says as much. Everything the
...
## because [member RlCar.is_ai] is what the match reads to know whose car
...
func _update_role() -> void:
	var mates: Array[RlCar] = _team_mates()
	if mates.size() <= 1:
		_set_role(Role.ATTACK)
		return
	var closest: RlCar = null
	var deepest: RlCar = null
	var best_time: float = INF
	var deepest_distance: float = -INF
	for mate: RlCar in mates:
	...
	match role:
		...
		_:
			if _car.boost < BOOST_HUNT_LEVEL:
				var pad: RlBoostPad = _nearest_pad()
				if pad != null:
			...
			behind.x = clampf(ball_at.x * 0.5, -RlConst.ARENA_EXTENT_X + 6.0,
				RlConst.ARENA_EXTENT_X - 6.0)
			behind.y = 0.0
			return behind
...
func _team_mates() -> Array[RlCar]:
	var out: Array[RlCar] = []
	for node: Node in get_tree().get_nodes_in_group(&"rl_cars"):
		var car: RlCar = node as RlCar
...
func _nearest_pad() -> RlBoostPad:
	var best: RlBoostPad = null
	var best_score: float = -INF
	for node: Node in get_tree().get_nodes_in_group(&"rl_boost_pads"):
		var pad: RlBoostPad = node as RlBoostPad
...
func _find_ball() -> RlBall:
	var balls: Array[Node] = get_tree().get_nodes_in_group(&"rl_ball")
	return balls[0] as RlBall if not balls.is_empty() else null
