...
func _ready() -> void:
	banner.text = ""
	boost_bar.max_value = RlConst.BOOST_MAX
...
func _on_state_changed(state: RlMatch.State) -> void:
	...
	_is_playing = state == RlMatch.State.PLAYING
	_refresh_ball_cam()
	match state:
		RlMatch.State.COUNTDOWN:
			banner_timer.stop()
			banner.text = "Kickoff"
		RlMatch.State.PLAYING:
			banner.text = ""
		RlMatch.State.REPLAY:
			banner.text = "Goal"
		RlMatch.State.OVER:
			banner_timer.stop()
...
func _on_goal_scored(team: RlCar.Team, scorer: RlCar) -> void:
	var side: String = "Blue" if team == RlCar.Team.BLUE else "Orange"
	var who: String = ""
...
func _on_match_ended(winner: RlCar.Team, tied: bool) -> void:
	if tied:
		banner.text = "Draw"
		return
	banner.text = "%s Wins" % ["Blue" if winner == RlCar.Team.BLUE else "Orange"]
