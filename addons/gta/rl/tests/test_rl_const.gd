...
func test_unreal_units_convert_to_metres_at_one_centimetre_each() -> void:
	assert_almost_eq(RlConst.ARENA_EXTENT_X, 4096.0 * RlConst.UU, 0.0001,
		"ARENA_EXTENT_X is 4096 uu")
	assert_almost_eq(RlConst.ARENA_EXTENT_Y, 5120.0 * RlConst.UU, 0.0001,
		"ARENA_EXTENT_Y is 5120 uu")
	assert_almost_eq(RlConst.ARENA_HEIGHT, 2048.0 * RlConst.UU, 0.0001,
		"ARENA_HEIGHT is 2048 uu")
	assert_almost_eq(RlConst.BALL_RADIUS, 91.25 * RlConst.UU, 0.0001,
		"BALL_COLLISION_RADIUS_SOCCAR is 91.25 uu")
	assert_almost_eq(RlConst.GRAVITY, 650.0 * RlConst.UU, 0.0001,
		"GRAVITY_Z is 650 uu/s^2")


func test_the_pitch_is_the_published_size() -> void:
	assert_almost_eq(RlConst.ARENA_EXTENT_X * 2.0, 81.92, 0.001, "81.92 m wall to wall")
	assert_almost_eq(RlConst.ARENA_EXTENT_Y * 2.0, 102.4, 0.001, "102.4 m goal to goal")
...
func test_the_corners_account_for_the_published_corner_wall_length() -> void:
	...
	var bite_along_x: float = (RlConst.ARENA_EXTENT_X * 2.0 - RlConst.BACK_WALL_LENGTH) * 0.5
	var bite_along_z: float = (RlConst.ARENA_EXTENT_Y * 2.0 - RlConst.SIDE_WALL_LENGTH) * 0.5
	assert_almost_eq(bite_along_x, bite_along_z, 0.001,
		"A 45 degree chamfer cuts the same distance off each axis")
	assert_almost_eq(bite_along_x * sqrt(2.0), RlConst.CORNER_WALL_LENGTH, 0.001,
		"CORNER_WALL_LENGTH 1629.174 uu is that cut on the diagonal")


func test_the_goal_line_sits_just_beyond_the_back_wall() -> void:
	assert_gt(RlConst.GOAL_LINE_Y, RlConst.ARENA_EXTENT_Y,
		"SOCCAR_GOAL_SCORE_BASE_THRESHOLD_Y 5124.25 uu is past the wall at 5120")


func test_a_full_tank_lasts_three_seconds() -> void:
	assert_almost_eq(RlConst.BOOST_MAX / RlConst.BOOST_PER_SECOND, 3.0, 0.01,
		"BOOST_MAX 100 at BOOST_USED_PER_SECOND 33.33")
	assert_almost_eq(RlConst.BOOST_START, RlConst.BOOST_MAX / 3.0, 0.01,
		"BOOST_SPAWN_AMOUNT is a third of a tank")


func test_boosting_accelerates_harder_in_the_air_than_on_the_ground() -> void:
	assert_gt(RlConst.BOOST_ACCEL_AIR, RlConst.BOOST_ACCEL_GROUND,
		"BOOST_ACCEL_AIR 3175/3 beats BOOST_ACCEL_GROUND 2975/3")


func test_the_published_pad_tables_are_all_there() -> void:
	assert_eq(RlConst.BIG_PADS.size(), 6, "LOCS_AMOUNT_BIG is 6")
	assert_eq(RlConst.SMALL_PADS.size(), 28, "LOCS_AMOUNT_SMALL_SOCCAR is 28")


func test_every_pad_lands_inside_the_pitch() -> void:
	for spot: Vector2 in RlConst.BIG_PADS + RlConst.SMALL_PADS:
		assert_lt(absf(spot.x), RlConst.ARENA_EXTENT_X, "Pad %s is inside the side walls" % spot)
		assert_lt(absf(spot.y), RlConst.ARENA_EXTENT_Y, "Pad %s is inside the back walls" % spot)
...
func test_the_pad_tables_are_symmetrical_end_to_end() -> void:
	...
	for spot: Vector2 in RlConst.SMALL_PADS:
		...
		for other: Vector2 in RlConst.SMALL_PADS:
...
func test_there_are_five_kickoff_spots_and_four_respawn_spots() -> void:
	assert_eq(RlConst.KICKOFF_SPOTS.size(), 5, "CAR_SPAWN_LOCATION_AMOUNT is 5")
	assert_eq(RlConst.RESPAWN_SPOTS.size(), 4, "CAR_RESPAWN_LOCATION_AMOUNT is 4")
...
func test_every_kickoff_spot_is_in_the_blue_half() -> void:
	...
	for spot: Vector3 in RlConst.KICKOFF_SPOTS:
...
func test_the_axis_conversion_puts_blue_on_the_positive_z_end() -> void:
	...
	var spawn: Vector3 = RlConst.to_godot(0.0, -46.08, 0.17)
...
func test_the_yaw_conversion_turns_a_kickoff_to_face_the_far_goal() -> void:
	...
	var facing: float = RlConst.to_godot_yaw(PI * 0.5)
	...
	var corner: Vector3 = Basis(Vector3.UP, RlConst.to_godot_yaw(PI * 0.25)).z
...
func test_a_curve_holds_its_end_values_beyond_either_end() -> void:
	assert_almost_eq(RlConst.curve(RlConst.STEER_CURVE, -10.0), 0.53356, 0.0001,
		"Below the first point the curve holds its first value")
	assert_almost_eq(RlConst.curve(RlConst.STEER_CURVE, 9000.0), 0.03454, 0.0001,
...
func test_a_curve_blends_between_its_points() -> void:
	...
	assert_almost_eq(RlConst.curve(RlConst.STEER_CURVE, 2.5), expected, 0.0001)


func test_an_empty_curve_reads_zero_rather_than_failing() -> void:
	var nothing: Array[Vector2] = []
	assert_almost_eq(RlConst.curve(nothing, 12.0), 0.0, 0.0001)


func test_the_throttle_curve_dies_at_the_speed_boost_is_needed_past() -> void:
	assert_almost_eq(RlConst.curve(RlConst.DRIVE_TORQUE_CURVE, 0.0), 1.0, 0.0001,
		"Full torque from a standstill")
	assert_almost_eq(RlConst.curve(RlConst.DRIVE_TORQUE_CURVE,
		RlConst.CAR_MAX_DRIVE_SPEED), 0.0, 0.0001,
...
func test_steering_lock_tightens_as_the_car_speeds_up() -> void:
	...
	for speed: float in [0.0, 5.0, 10.0, 15.0, 17.5, 30.0]:
		var lock: float = RlConst.curve(RlConst.STEER_CURVE, speed)
...
func test_a_powerslide_widens_the_lock_at_the_speeds_it_is_used_at() -> void:
	...
	for speed: float in [5.0, 10.0, 15.0, 20.0]:
		assert_gt(RlConst.curve(RlConst.POWERSLIDE_STEER_CURVE, speed),
			RlConst.curve(RlConst.STEER_CURVE, speed),
...
func test_below_walking_pace_gripping_turns_harder_than_sliding() -> void:
	...
	assert_gt(RlConst.curve(RlConst.STEER_CURVE, 0.0),
		RlConst.curve(RlConst.POWERSLIDE_STEER_CURVE, 0.0))
...
func test_roll_is_the_fastest_axis_in_the_air() -> void:
	...
	assert_gt(RlConst.AIR_CONTROL_TORQUE.z, RlConst.AIR_CONTROL_TORQUE.x)
	assert_gt(RlConst.AIR_CONTROL_TORQUE.z, RlConst.AIR_CONTROL_TORQUE.y)
...
func test_air_control_settles_under_the_angular_speed_cap() -> void:
	...
	for axis: int in 3:
		var settled: float = RlConst.AIR_CONTROL_TORQUE[axis] / RlConst.AIR_CONTROL_DAMPING[axis]
		assert_lt(settled, 10.0, "Axis %d settles at %f rad/s" % [axis, settled])


func test_a_backward_dodge_carries_furthest() -> void:
	assert_gt(RlConst.FLIP_BACKWARD_MAX_SCALE, RlConst.FLIP_SIDE_MAX_SCALE)
	assert_gt(RlConst.FLIP_SIDE_MAX_SCALE, RlConst.FLIP_FORWARD_MAX_SCALE)


func test_supersonic_is_held_below_the_speed_it_starts_at() -> void:
	assert_lt(RlConst.SUPERSONIC_MAINTAIN, RlConst.SUPERSONIC_START,
		"Dropping under the threshold stays supersonic for a second")


func test_the_gaps_in_the_sourcing_are_written_down() -> void:
	assert_gt(RlConst.UNSOURCED.size(), 0,
