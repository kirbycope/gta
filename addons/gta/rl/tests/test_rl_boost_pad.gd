...
const BOOST_PAD: PackedScene = preload("res://addons/gta/rl/scenes/rl_boost_pad.tscn")
...
func test_a_small_pad_is_worth_twelve_and_a_big_one_a_full_tank() -> void:
	pad.is_big = false
	assert_almost_eq(pad.amount(), RlConst.PAD_BOOST_SMALL, 0.01, "BOOST_AMOUNT_SMALL")
	pad.is_big = true
	assert_almost_eq(pad.amount(), RlConst.PAD_BOOST_BIG, 0.01, "BOOST_AMOUNT_BIG")
...
func test_a_big_pad_fills_the_tank_from_anywhere() -> void:
	...
	pad._on_body_entered(car)
	assert_almost_eq(car.boost, RlConst.BOOST_MAX, 0.01)
...
func test_the_cooldown_matches_the_kind_of_pad() -> void:
	...
	car.refill_boost(0.0)
	pad._on_body_entered(car)
	assert_almost_eq(pad.get_node(^"Cooldown").wait_time, RlConst.PAD_COOLDOWN_BIG, 0.01,
...
func test_a_small_pad_comes_back_sooner() -> void:
	...
	car.refill_boost(0.0)
	pad._on_body_entered(car)
	assert_almost_eq(pad.get_node(^"Cooldown").wait_time, RlConst.PAD_COOLDOWN_SMALL, 0.01,
