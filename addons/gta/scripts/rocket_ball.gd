...
##
## Every constant comes from [RocketConst]. Two things here are worth knowing
...
signal touched(car: RocketCar) ## A car hit the ball; the match credits goals from this.
...
var _last_touch_by: RocketCar = null ## Who hit it last, for crediting a goal.
var _touch_cooldown: Dictionary = {} ## Car to seconds left, so one hit is not counted sixty times.


func _ready() -> void:
	mass = RocketConst.BALL_MASS
	...
	linear_damp = RocketConst.BALL_DRAG
	angular_damp_mode = RigidBody3D.DAMP_MODE_REPLACE
	angular_damp = 0.0
	contact_monitor = true
	max_contacts_reported = 8
	can_sleep = false
	add_to_group(&"rocket_ball")
...
func _physics_process(delta: float) -> void:
	...
	if linear_velocity.length() > RocketConst.BALL_MAX_SPEED:
		linear_velocity = linear_velocity.normalized() * RocketConst.BALL_MAX_SPEED
	if angular_velocity.length() > RocketConst.BALL_MAX_ANGULAR:
		angular_velocity = angular_velocity.normalized() * RocketConst.BALL_MAX_ANGULAR


## Who touched the ball last, which is who gets the goal.
func last_touch() -> RocketCar:
...
func predict(seconds: float) -> Vector3:
	...
	for _i: int in steps:
		velocity.y -= RocketConst.GRAVITY * PREDICT_STEP
		velocity -= velocity * RocketConst.BALL_DRAG * PREDICT_STEP
...
func _bounce(at: Vector3, velocity: Vector3) -> Dictionary:
	var radius: float = RocketConst.BALL_RADIUS
	var limit_x: float = RocketConst.ARENA_EXTENT_X - radius
	var limit_z: float = RocketConst.ARENA_EXTENT_Y - radius
	var ceiling: float = RocketConst.ARENA_HEIGHT - radius
	if at.y < radius:
		at.y = radius
		velocity.y = absf(velocity.y) * RocketConst.BALL_RESTITUTION
	elif at.y > ceiling:
		at.y = ceiling
		velocity.y = -absf(velocity.y) * RocketConst.BALL_RESTITUTION
	if absf(at.x) > limit_x:
		at.x = signf(at.x) * limit_x
		velocity.x = -absf(velocity.x) * signf(at.x) * RocketConst.BALL_RESTITUTION
	# the goal mouths are open, so the ball only meets the back wall outside them
	if absf(at.z) > limit_z and absf(at.x) > RocketConst.GOAL_HALF_WIDTH:
		at.z = signf(at.z) * limit_z
		velocity.z = -absf(velocity.z) * signf(at.z) * RocketConst.BALL_RESTITUTION
...
## [RocketConst]; the order they are combined in is read off RocketSim's
...
func _on_body_entered(body: Node) -> void:
	...
	if _touch_cooldown.has(car):
		return
	_touch_cooldown[car] = RocketConst.BUMP_COOLDOWN
	_last_touch_by = car
	touched.emit(car)
	...
	direction.y *= RocketConst.BALL_CAR_IMPULSE_Z_SCALE
	if direction.length_squared() <= 0.0:
		return
	direction = direction.normalized()
	direction = (direction + car.nose() * RocketConst.BALL_CAR_IMPULSE_FORWARD_SCALE).normalized()
	var scale_factor: float = RocketConst.curve(RocketConst.BALL_CAR_IMPULSE_CURVE, closing)
	var delta_velocity: float = minf(closing * scale_factor, RocketConst.BALL_CAR_IMPULSE_MAX_DELTA)
