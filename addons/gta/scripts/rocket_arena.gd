...
extends Node3D
## The soccar pitch, built from the published dimensions in [RocketConst].
##
...
func _build_surfaces() -> void:
	var half_x: float = RocketConst.ARENA_EXTENT_X
	var half_z: float = RocketConst.ARENA_EXTENT_Y
	var height: float = RocketConst.ARENA_HEIGHT
	...
	for side: int in [-1, 1]:
		_box("SideWall%s" % ("Left" if side < 0 else "Right"),
			Vector3(WALL_THICKNESS, height, RocketConst.SIDE_WALL_LENGTH),
	...
	var post: float = RocketConst.GOAL_HALF_WIDTH
	var crossbar: float = RocketConst.GOAL_HEIGHT
	for end: int in [-1, 1]:
		var z: float = end * (half_z + WALL_THICKNESS * 0.5)
		var panel_width: float = (RocketConst.BACK_WALL_LENGTH * 0.5) - post
		for side: int in [-1, 1]:
...
## sticky force in [RocketCar], a car can carry speed off the floor and up onto
...
func _build_fillets() -> void:
	...
	var half_z: float = RocketConst.ARENA_EXTENT_Y
	var post: float = RocketConst.GOAL_HALF_WIDTH
	var back_half: float = RocketConst.BACK_WALL_LENGTH * 0.5
	...
	for side: int in [-1, 1]:
		_fillet("FilletSide%s" % ("Left" if side < 0 else "Right"),
			Vector3(side * half_x, 0.0, 0.0), RocketConst.SIDE_WALL_LENGTH,
...
func _build_goals() -> void:
	var post: float = RocketConst.GOAL_HALF_WIDTH
	var crossbar: float = RocketConst.GOAL_HEIGHT
	var depth: float = RocketConst.GOAL_DEPTH
	for end: int in [-1, 1]:
		var mouth: float = end * RocketConst.ARENA_EXTENT_Y
...
func _build_markings() -> void:
	var line: CSGBox3D = _box("HalfwayLine",
		Vector3(RocketConst.ARENA_EXTENT_X * 2.0, 0.02, LINE_WIDTH),
...
func _build_boost_pads() -> void:
	var scene: PackedScene = load("res://addons/gta/scenes/rocket_boost_pad.tscn") as PackedScene
	if scene == null:
		push_warning("RocketArena: rocket_boost_pad.tscn is missing, so the pitch has no boost.")
		return
	var pads: Node3D = Node3D.new()
	pads.name = "BoostPads"
	add_child(pads)
	for spot: Vector2 in RocketConst.BIG_PADS:
...
func _add_pad(parent: Node3D, scene: PackedScene, spot: Vector2, big: bool) -> void:
	var pad: RocketBoostPad = scene.instantiate() as RocketBoostPad
	if pad == null:
		return
	pad.is_big = big
	# the tables are in Rocket League's axes, where y runs along the pitch
	pad.position = RocketConst.to_godot(spot.x, spot.y, 0.0)
