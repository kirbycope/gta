class_name Tm2Projectile
extends Area3D
## One Twisted Metal shot in flight: a missile, a bomb or a machine gun round.
##
## Damage, speed and homing strength come from [Tm2Roster.WEAPONS], so the
...
func configure(weapon_key: StringName, from: Node3D, at: Node3D, gun_damage: float) -> void:
	...
	shooter = from
	target = at
	var stats: Dictionary = Tm2Roster.WEAPONS.get(weapon_key, {})
	if stats.is_empty():
		damage = gun_damage
...
func _on_body_entered(body: Node3D) -> void:
	if body == shooter:
		return
	var combat: CarCombat = _combat_of(body)
	if combat != null:
...
func _combat_of(body: Node3D) -> CarCombat:
	if not is_instance_valid(body):
		return null
	for child: Node in body.get_children():
		if child is CarCombat:
			return child as CarCombat
	return null
