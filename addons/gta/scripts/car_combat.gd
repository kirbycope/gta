...
const PROJECTILE: PackedScene = preload("res://addons/gta/scenes/tm2_projectile.tscn")
const MACHINE_GUN_DAMAGE: float = 1.0 ## Not published anywhere; see Tm2Roster.UNSOURCED.
const MACHINE_GUN_INTERVAL: float = 0.1
const HEALTH_TIERS: int = 3 ## AICarUpdateHealthTier splits health into thirds.

@export var car: StringName = &"roadkill" ## Roster key, see [Tm2Roster].
...
func pick_up(weapon: StringName, count: int = 1) -> void:
	if not Tm2Roster.WEAPONS.has(weapon):
		return
...
func cycle_weapon() -> void:
	var keys: Array = Tm2Roster.WEAPONS.keys()
