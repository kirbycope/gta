...
## own published numbers in [RocketConst]. The order of the step follows
...
## [Vehicle] defines. A [RocketAi] reaches it through [method
## set_rocket_input].
...
signal bumped(other: RocketCar) ## Knocked another car about without wrecking it.
...
signal demolished(by: RocketCar) ## Wrecked by an opponent at speed.
signal respawned() ## Back in play after [constant RocketConst.DEMO_RESPAWN_TIME].

## Which end of the pitch this car defends. Blue defends positive z in Godot's
## axes, which is Rocket League's negative y; see [method RocketConst.to_godot].
...
var boost: float = RocketConst.BOOST_START: set = _set_boost
...
func _ready() -> void:
	super()
	mass = RocketConst.CAR_MASS
	contact_monitor = true
	max_contacts_reported = 8
	can_sleep = false
	add_to_group(&"rocket_cars")
	add_to_group(&"rocket_blue" if team == Team.BLUE else &"rocket_orange")
...
func target_goal() -> Vector3:
	return Vector3(0.0, RocketConst.GOAL_HEIGHT * 0.5, RocketConst.GOAL_LINE_Y * attack_direction())
...
func refill_boost(to: float = RocketConst.BOOST_START) -> void:
	boost = to


## Add boost from a pad, capped at a full tank.
func collect_boost(amount: float) -> void:
	boost = minf(boost + amount, RocketConst.BOOST_MAX)
...
func demolish(by: RocketCar = null) -> void:
	if is_demolished:
		return
	is_demolished = true
	_demo_timer = RocketConst.DEMO_RESPAWN_TIME
	freeze = true
	visible = false
...
func _physics_process(delta: float) -> void:
	...
	on_ground = wheels_down >= RocketConst.WHEELS_FOR_GROUND
	...
	if on_ground:
		...
		if _flip_time >= 0.0 and _flip_time > RocketConst.FLIP_TORQUE_MIN_TIME:
			_flip_time = -1.0
	...
	if wheels_down < wheels.size() and absf(_throttle) > RocketConst.THROTTLE_DEADZONE:
		_apply_auto_roll(delta)
	_tick_boost(delta)
	_tick_supersonic(delta)

	if linear_velocity.length() > RocketConst.CAR_MAX_SPEED:
		linear_velocity = linear_velocity.normalized() * RocketConst.CAR_MAX_SPEED
	if angular_velocity.length() > RocketConst.CAR_MAX_ANGULAR:
		angular_velocity = angular_velocity.normalized() * RocketConst.CAR_MAX_ANGULAR
...
func _apply_sticky_force() -> void:
	...
	var rolling: bool = absf(linear_velocity.dot(nose())) > RocketConst.STOPPING_SPEED
	var scale: float = RocketConst.STICKY_FORCE_BASE
	if absf(_throttle) > RocketConst.THROTTLE_DEADZONE or rolling:
		scale += 1.0 - absf(up.y)
	...
	apply_central_force(-up * scale * RocketConst.GRAVITY * mass)
...
func _apply_auto_roll(delta: float) -> void:
	...
	angular_velocity += align * RocketConst.AUTOROLL_TORQUE * delta
	apply_central_force(-up * RocketConst.AUTOROLL_FORCE * mass)
...
func _drive(delta: float) -> void:
	...
	_slide = clampf(_slide + (RocketConst.POWERSLIDE_RISE_RATE if _powerslide
		else -RocketConst.POWERSLIDE_FALL_RATE) * delta, 0.0, 1.0)
	var lock: float = lerpf(
		RocketConst.curve(RocketConst.STEER_CURVE, forward_speed),
		RocketConst.curve(RocketConst.POWERSLIDE_STEER_CURVE, forward_speed),
		_slide)
	...
	var coasting: float = 1.0 if absf(_throttle) > RocketConst.THROTTLE_DEADZONE \
		else RocketConst.curve(RocketConst.NON_STICKY_FRICTION_CURVE, absf(_surface_up().y))
	for wheel: VehicleWheel3D in wheels:
		if wheel == null:
			continue
		wheel.wheel_friction_slip = _base_friction * coasting * lerpf(
			1.0, RocketConst.HANDBRAKE_LAT_FRICTION, _slide if not wheel.use_as_steering else _slide * 0.5)

	var drive: float = 0.0
	var stop: float = 0.0
	if absf(_throttle) > RocketConst.THROTTLE_DEADZONE:
		var torque_factor: float = RocketConst.curve(RocketConst.DRIVE_TORQUE_CURVE, forward_speed)
		drive = mass * RocketConst.THROTTLE_ACCEL * torque_factor * _throttle
		...
		if not is_zero_approx(speed) and signf(_throttle) != signf(speed) \
				and forward_speed > RocketConst.STOPPING_SPEED:
			stop = mass * RocketConst.BRAKE_ACCEL
			drive = 0.0
	else:
		# coasting is a gentle brake, and becomes a full one once nearly stopped
		stop = mass * (RocketConst.BRAKE_ACCEL if forward_speed <= RocketConst.STOPPING_SPEED
			else RocketConst.COAST_ACCEL)
...
func _air_control(delta: float) -> void:
	...
	for axis: int in 3:
		var accel: float = RocketConst.AIR_CONTROL_TORQUE[axis] * inputs[axis] \
			- RocketConst.AIR_CONTROL_DAMPING[axis] * spin[axis] * (1.0 - absf(inputs[axis]))
		change += axes[axis] * accel
	angular_velocity += change * delta

	if absf(_throttle) > RocketConst.THROTTLE_DEADZONE:
		apply_central_force(nose() * mass * RocketConst.AIR_THROTTLE_ACCEL * _throttle)
...
func _tick_jump(delta: float, pressed: bool) -> void:
	...
	if _is_jumping:
		_jump_time += delta
		if _jump_held and _jump_time <= RocketConst.JUMP_MAX_TIME:
			apply_central_force(up * mass * RocketConst.JUMP_ACCEL)
		elif _jump_time >= RocketConst.JUMP_MIN_TIME:
			_is_jumping = false
	...
	if on_ground:
		...
		_jump_time = 0.0
		_has_double_jump = true
		apply_central_impulse(up * mass * RocketConst.JUMP_IMMEDIATE)
		jumped.emit(false)
		return
	if not _has_double_jump or _airborne_for > RocketConst.DOUBLE_JUMP_MAX_DELAY:
		return
	if _autoflip_time > 0.0:
		return
	_has_double_jump = false
	# the deadzone is the sum of the three stick axes, not any one of them
	if absf(_yaw) + absf(_pitch) + absf(_roll) >= RocketConst.FLIP_DEADZONE:
		_start_flip()
	else:
		apply_central_impulse(up * mass * RocketConst.JUMP_IMMEDIATE)
		jumped.emit(true)
...
func _tick_autoflip(delta: float, pressed: bool) -> void:
	if _autoflip_time > 0.0:
		...
		angular_velocity += nose() * RocketConst.AUTOFLIP_TORQUE * _autoflip_direction * delta
		return
	...
	if hit.is_empty() or (hit["normal"] as Vector3).y < RocketConst.AUTOFLIP_NORMAL_MIN:
		return
	var roll: float = global_rotation.z
	if absf(roll) <= RocketConst.AUTOFLIP_ROLL_MIN:
		return
	_autoflip_time = RocketConst.AUTOFLIP_TIME * (absf(roll) / PI)
	_autoflip_direction = 1.0 if roll > 0.0 else -1.0
	apply_central_impulse(-roof() * mass * RocketConst.AUTOFLIP_IMPULSE)
	auto_flipped.emit()
...
func _start_flip() -> void:
	...
	var speed_ratio: float = absf(forward_speed) / RocketConst.CAR_MAX_SPEED
	var backwards: bool = direction.x < 0.0 if absf(forward_speed) < 1.0 \
		else (direction.x >= 0.0) != (forward_speed >= 0.0)
	var along: float = RocketConst.FLIP_BACKWARD_MAX_SCALE if backwards \
		else RocketConst.FLIP_FORWARD_MAX_SCALE

	var throw: Vector2 = direction * RocketConst.FLIP_INITIAL_VEL
	throw.x *= (along - 1.0) * speed_ratio + 1.0
	throw.y *= (RocketConst.FLIP_SIDE_MAX_SCALE - 1.0) * speed_ratio + 1.0
	if backwards:
		throw.x *= RocketConst.FLIP_BACKWARD_SCALE_X
	...
	_flip_torque = Vector3(RocketConst.FLIP_TORQUE_PITCH * direction.x, 0.0,
		RocketConst.FLIP_TORQUE_ROLL * direction.y)
	_flip_time = 0.0
	_pitchlock_time = RocketConst.FLIP_PITCHLOCK_TIME
	dodged.emit()
...
func _tick_flip(delta: float) -> void:
	if _flip_time < 0.0:
		return
	_flip_time += delta
	if _flip_time >= RocketConst.FLIP_TORQUE_TIME:
		_flip_time = -1.0
		return
	...
	if _flip_time >= RocketConst.FLIP_Z_DAMP_START and _flip_time <= RocketConst.FLIP_Z_DAMP_END \
...
func _tick_boost(delta: float) -> void:
	...
	if wants and _boost_min_timer <= 0.0:
		_boost_min_timer = RocketConst.BOOST_MIN_TIME
	var burning: bool = wants or _boost_min_timer > 0.0
	_update_boost_sound(burning)
	if not burning:
		return
	boost = maxf(0.0, boost - RocketConst.BOOST_PER_SECOND * delta)
	if linear_velocity.length() >= RocketConst.CAR_MAX_SPEED:
		return
	var accel: float = RocketConst.BOOST_ACCEL_GROUND if on_ground else RocketConst.BOOST_ACCEL_AIR
...
func _tick_supersonic(delta: float) -> void:
	...
	if speed >= RocketConst.SUPERSONIC_START:
		is_supersonic = true
		_supersonic_timer = RocketConst.SUPERSONIC_MAINTAIN_TIME
	elif is_supersonic and speed >= RocketConst.SUPERSONIC_MAINTAIN:
...
func _tick_respawn(delta: float) -> void:
	...
	var spot: Vector3 = RocketConst.RESPAWN_SPOTS[randi() % RocketConst.RESPAWN_SPOTS.size()]
	var side: float = attack_direction()
	place_at(
		Vector3(spot.x * -side, RocketConst.CAR_RESPAWN_HEIGHT, spot.y * -side),
		RocketConst.to_godot_yaw(spot.z) + (0.0 if team == Team.BLUE else PI))
	refill_boost()
	respawned.emit()
...
func _on_body_entered(body: Node) -> void:
	var other: RocketCar = body as RocketCar
	...
	if toward.dot(nose()) < RocketConst.BUMP_MIN_FORWARD_DIST:
		return
	_bump_cooldowns[other] = RocketConst.BUMP_COOLDOWN

	if is_supersonic and other.team != team:
		other.demolish(self)
		return
	var curve: Array[Vector2] = RocketConst.BUMP_GROUND_CURVE if other.on_ground \
		else RocketConst.BUMP_AIR_CURVE
	...
	other.linear_velocity += travel * RocketConst.curve(curve, closing) \
		+ up * RocketConst.curve(RocketConst.BUMP_UP_CURVE, closing)
	bumped.emit(other)
...
func _apply_rocket_camera() -> void:
	if not is_instance_valid(chase_camera):
		return
	chase_camera.pivot_height = RocketConst.CAMERA_HEIGHT
	chase_camera.pitch = deg_to_rad(RocketConst.CAMERA_ANGLE)
	var arm: SpringArm3D = chase_camera.get_node_or_null(^"SpringArm3D") as SpringArm3D
	if arm != null:
		arm.spring_length = RocketConst.CAMERA_DISTANCE
	if is_instance_valid(camera):
		camera.fov = RocketConst.CAMERA_FOV
...
func _update_boost_sound(burning: bool) -> void:
	...
	sfx_boost.pitch_scale = lerpf(BOOST_PITCH_RANGE.x, BOOST_PITCH_RANGE.y,
		clampf(linear_velocity.length() / RocketConst.CAR_MAX_SPEED, 0.0, 1.0))
...
func _on_bumped(_other: RocketCar) -> void:
	if is_instance_valid(sfx_impact):
		sfx_impact.play()
...
func _find_ball() -> Node3D:
	if not is_inside_tree():
		return null
	return get_tree().get_first_node_in_group(&"rocket_ball") as Node3D


func _set_boost(value: float) -> void:
	var clamped: float = clampf(value, 0.0, RocketConst.BOOST_MAX)
	if is_equal_approx(clamped, boost):
		return
	boost = clamped
	boost_changed.emit(boost, RocketConst.BOOST_MAX)
