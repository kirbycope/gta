class_name GtaCar
extends Vehicle
## Godot Tim's Automobile: the road car, with a GTA V style handling model.
##
## Everything here is what makes this car this car, and none of it is shared: a five-gear transmission,
## a traction curve, drive and brake bias, a handbrake slide, counter-steer assist, drag, downforce and
## anti-roll bars, RPM-driven engine audio, flip and burn and explode damage (when the scene gives it fire
## and explosion nodes), a first-person view, a speedometer and a car radio. The chassis under it, the
## wheels, the rideable contract and the multiplayer hand-off, are [Vehicle]'s.
##
## The Player gets in with "action" and drives through their own [Riding] state, which plays the enter and
## exit clips named on this scene, hands the view to [member Vehicle.camera] and turns the Player's
## collision off for the seat.
##
## This is the one car in the addon that seats its driver itself rather than through
## [member Vehicle.seat]. Its get-in clip is authored to start at the kerb and walk the driver into the
## car, so a scene [code]Seat[/code] node, which the [Riding] state pins to the instant they get on, would
## play that walk from inside the seat. It has none, and [method ride] puts them on
## [member driver_seat] every frame instead.
##
## Over the network VehicleSynchronizer carries its transform, drive state, damage,
## [member Vehicle.current_driver_peer_id] and [member radio_station] to the other peers.
...
var menu_displayed: bool = false ## The prompt is up for [member Vehicle.player], who is inside PlayerDetection.
...
@onready var driver_seat: Node3D = $DriverSeat ## Where [method ride] puts the driver, since this car has no scene "Seat".
...
# Rideable contract: the Player's Riding state calls these

## Rideable contract: the Riding state hands the Player over at the door; it plays
## [member Vehicle.mount_animation], which walks them in, and the chase camera starts behind the car.
func mount(rider: Player) -> void:
	_hide_prompt()
	super(rider)
	rider.global_position = enter_car.global_position
	rider.orientation = enter_car.global_transform
	rider.orientation.origin = Vector3.ZERO
	rider.player_model.global_transform = enter_car.global_transform
...
##
## This car seats its driver here rather than through [member Vehicle.seat], because the get-in clip is
## authored to walk them in from the kerb and a scene "Seat" node would be pinned to before it plays.
...
## as they get in, and getting out drops the first-person view and ends the drive.
...
func _physics_process(delta: float) -> void:
	...
	if is_driving_this_car and not is_on_fire and not has_exploded:
		...
		if not freeze and linear_velocity.length() < 0.5 and angular_velocity.length() < 0.5 and wheels_in_contact() > 0:
			freeze = true
...
func _apply_drivetrain(delta: float) -> void:
	...
	var is_grounded: bool = wheels_in_contact() > 0
...
func _update_engine_sfx() -> void:
	...
	var is_screeching: bool = wheels_in_contact() > 0 and speed > min_brake_sound_velocity \
...
# The door, and the prompt that gets the Player in
