...
## Purpose: the road car. It is ridden through the Player's Riding state: mounting seats the driver behind the
## enter animation, drive inputs reach the drivetrain, the chase camera takes the view, and the exit action gets
## out. The speedometer, the door, the action prompt and the radio are all this car's own rather than the
## chassis's; the hand-off underneath them belongs to [Vehicle] and is tested in test_vehicle.gd.

const DEMO_SCENE: PackedScene = preload("res://addons/gta/gta/scenes/demo.tscn")
...
## The synchronizer is the road car's, not the chassis's, so the assertion that it changes hands with
## the car lives with the road car rather than in the base's test.
func test_the_synchronizer_changes_hands_with_the_car() -> void:
	player.set_multiplayer_authority(42)
	car.set_driver(player)
	assert_eq(car.vehicle_synchronizer.get_multiplayer_authority(), 42,
		"so the car's state is the driver's to send")
	car.set_driver(null)
	assert_eq(car.vehicle_synchronizer.get_multiplayer_authority(), Vehicle.SERVER_PEER)


## Moved here from test_car_combat.gd, which had it only to contrast with the Twisted Metal car.
func test_the_road_car_shows_its_driver() -> void:
	assert_false(car.hides_driver_model, "The CR-V has a cabin to seat the driver in")
