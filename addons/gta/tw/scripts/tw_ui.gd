...
## [RlUi] uses: [TwCombat] emits health and the weapon, [TwCar] emits the turbo meter.
...
func watch(car: TwCar) -> void:
	if not is_instance_valid(car):
		return
	car.turbo_changed.connect(_on_turbo_changed)
	_on_turbo_changed(car.turbo)
	var combat: TwCombat = car.get_node_or_null(^"TwCombat") as TwCombat
	if combat == null:
		return
