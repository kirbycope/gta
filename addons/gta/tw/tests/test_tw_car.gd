...
## controls.
##
## The one thing here that is a measurement rather than a rule is [method TwCar.forward]. This
...
## drives a car on a floor and checks it really does travel the way [method TwCar.forward] claims,
...
func before_each() -> void:
	...
	car = TM2_CAR.instantiate() as TwCar
	car.car = &"roadkill"
	(car.get_node(^"TwAi") as TwAi).enabled = false
...
func test_it_reaches_the_published_top_speed_for_its_car_and_no_further() -> void:
	var published: float = TwRoster.top_speed(&"roadkill")
...
func test_turbo_raises_the_ceiling_to_the_published_turbo_speed() -> void:
	assert_almost_eq(TwRoster.top_speed(&"roadkill", true), car._turbo_speed, 0.01)
	car.set_drive_input(true, false, true, 0.0)
	await wait_physics_frames(2)
	assert_true(car.is_turbo_engaged, "The button is down and the meter is full")
	assert_almost_eq(car.speed_limit(), TwRoster.top_speed(&"roadkill", true), 0.01,
		"The turbo ceiling is the published turbo speed")
	assert_gt(car.speed_limit(), TwRoster.top_speed(&"roadkill"), "which is faster than the ordinary one")
...
func test_an_empty_meter_will_not_engage() -> void:
	...
	assert_almost_eq(car.speed_limit(), TwRoster.top_speed(&"roadkill"), 0.01,
		"so the ceiling is the ordinary one")


func test_a_wreck_stops_answering_the_controls() -> void:
	watch_signals(car)
	var combat: TwCombat = car.get_node(^"TwCombat") as TwCombat
...
func test_the_roster_key_names_the_car_in_both_places_it_matters() -> void:
	var minion: TwCar = TM2_CAR.instantiate() as TwCar
	minion.car = &"minion"
	(minion.get_node(^"TwCombat") as TwCombat).car = &"minion"
	(minion.get_node(^"TwAi") as TwAi).enabled = false
	add_child_autofree(minion)
	await wait_physics_frames(2)
	assert_almost_eq(minion.speed_limit(), TwRoster.top_speed(&"minion"), 0.01,
		"Minion is the fastest car in the roster and drives like it")
	assert_almost_eq((minion.get_node(^"TwCombat") as TwCombat).max_health, 230.0, 0.01,
		"and the toughest")
