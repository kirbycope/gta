...
const TM2_UI: PackedScene = preload("res://addons/gta/tw/scenes/tw_ui.tscn")
...
func before_each() -> void:
	ui = TM2_UI.instantiate() as TwUi
	add_child_autofree(ui)
	car = TM2_CAR.instantiate() as TwCar
	car.car = &"sweet_tooth"
	(car.get_node(^"TwCombat") as TwCombat).car = &"sweet_tooth"
...
func test_taking_a_hit_moves_the_armour_bar() -> void:
	(car.get_node(^"TwCombat") as TwCombat).take_hit(40.0)
...
func test_picking_up_a_weapon_names_it_and_counts_it() -> void:
	(car.get_node(^"TwCombat") as TwCombat).pick_up(&"homing_missile", 3)
...
func test_it_connects_rather_than_polling() -> void:
	...
	assert_true((car.get_node(^"TwCombat") as TwCombat).health_changed.is_connected(ui._on_health_changed))
