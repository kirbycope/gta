...
## everything it does reaches the car through [method TwCar.set_drive_input].
...
func before_each() -> void:
	waypoints = TwWaypoints.new()
	waypoints.points = PackedVector3Array([
		Vector3(0, 0, 0), Vector3(30, 0, 0), Vector3(30, 0, 30), Vector3(0, 0, 30),
	])
	car = TM2_CAR.instantiate()
	car.car = &"roadkill"
	(car.get_node(^"TwCombat") as TwCombat).car = &"roadkill"
	var d: TwAi = car.get_node(^"TwAi")
	d.enabled = true
	d.waypoints = waypoints
	add_child_autofree(car)
	driver = car.get_node(^"TwAi")
	combat = car.get_node(^"TwCombat")
...
func test_it_starts_out_of_battle_with_nobody_to_fight() -> void:
	assert_eq(driver.state, TwAi.State.OUT_OF_BATTLE)
...
func test_it_drives_to_the_published_top_speed_for_its_car() -> void:
	assert_almost_eq(driver._top_speed, TwRoster.top_speed(&"roadkill"), 0.01)
...
func test_an_empty_waypoint_set_reports_no_point_rather_than_crashing() -> void:
	var empty: TwWaypoints = TwWaypoints.new()
...
func test_a_disabled_driver_leaves_the_car_alone() -> void:
	var parked: TwCar = TM2_CAR.instantiate()
	(parked.get_node(^"TwAi") as TwAi).enabled = false
