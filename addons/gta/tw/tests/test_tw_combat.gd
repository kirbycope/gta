...
func before_each() -> void:
	...
	# set before the car enters the tree: TwCombat reads the roster in _ready
	car.car = &"sweet_tooth"
...
func test_the_health_tier_the_ai_reads_climbs_as_the_car_is_hurt() -> void:
	...
	assert_eq(combat.health_tier(), TwCombat.HEALTH_TIERS - 1, "Nearly dead is the top tier")
...
func test_a_fired_shot_flies_away_from_the_nose() -> void:
	...
	for child: Node in car.get_parent().get_children():
		if child is TwProjectile:
			before.append(child)
	combat.pick_up(&"fire_missile", 1)
	combat.fire_forward()
	await wait_physics_frames(1)
	var shot: TwProjectile = null
	for child: Node in car.get_parent().get_children():
		if child is TwProjectile and not before.has(child):
			shot = child as TwProjectile
			break
